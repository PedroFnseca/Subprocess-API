...
setInterval(async () => {
  ...
  if (now.getHours() === parseInt(process.env.HOUR_TO_VERIFY_LENDING)) {
    ...
    console.log(
      `║\t⊙ Os emprestimos serão verificados às ${process.env.HOUR_TO_VERIFY_LENDING}:00\t║`
    );
    console.log("║\t\t\t\t\t\t\t║");
  }
  ...
  if (now.getHours() === parseInt(process.env.HOUR_TO_VERIFY_PENALTY)) {
    ...
    console.log(
      `║\t⊙ As multas serão verificadas às ${process.env.HOUR_TO_VERIFY_PENALTY}:00\t\t║`
    );
  }
  ...
  console.log(
    `║      Verificações concluidas: ${now.toLocaleString("pt-BR")}\t║`
  );
  console.log("╚═══════════════════════════════════════════════════════╝\n\n");
